...
from app.models.movimentacao import Movimentacao, Tipodemovimentacao
...
def listar_movimentacoes(
    ...
    produtos      = db.query(Produto).filter(Produto.ativa == True).all()
...
def form_nova_movimentacao(
    ...
    return templates.TemplateResponse(
        ...
        "movimentacoes/form.html",
        {
            ...
            "produtos":   produtos,
            "produto_id": produto_id,
            "tipos":      Tipodemovimentacao,  # passa o enum para o template
        }
    )
...
def registrar_movimentacao(
    ...
    if tipo not in (Tipodemovimentacao.ENTRADA, Tipodemovimentacao.SAIDA):
        return templates.TemplateResponse(
            request,
            "movimentacoes/form.html",
            {
    ...
    if tipo == Tipodemovimentacao.SAIDA and quantidade > produto.estoque:
        return templates.TemplateResponse(
            request,
            "movimentacoes/form.html",
            {
    ...
    # ----------------------------------------------------------
    if tipo == Tipodemovimentacao.ENTRADA:
...
def historico_produto(
    ...
    total_entradas = sum(
        ...
        if m.tipo == Tipodemovimentacao.ENTRADA
    )
    total_saidas = sum(
        m.quantidade for m in movimentacoes
        if m.tipo == Tipodemovimentacao.SAIDA
    )
